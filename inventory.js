...
// En inventory.js - Corregir la función addToHistory
function addToHistory(action, productId, productName, oldQuantity, newQuantity) {
    try {
        // Obtener el historial existente de manera segura
        let history;
        try {
            history = JSON.parse(localStorage.getItem('history')) || [];
        } catch (e) {
            console.error('Error al leer el historial, inicializando nuevo', e);
            history = [];
        }
        
        // Verificar que history sea un array
        if (!Array.isArray(history)) {
            console.error('El historial no era un array, inicializando nuevo');
            history = [];
        }
        ...
        try {
            ...
            // Intentar con un historial más pequeño si hay error de almacenamiento
            if (e.name === 'QuotaExceededError') {
                localStorage.setItem('history', JSON.stringify(history.slice(-100))); // Guardar solo los últimos 100 registros
            }
        }
    } catch (e) {
